...
const ResetPassword: React.FC = () => {
  ...
  const [reset, { isError, isLoading }] = useSuperAdminResetPassMutation();
  const [adminReset, { isError: adminError, isLoading: adminLoading }] =
    useAdminResetPassMutation();
  ...
  const onSubmit: any = async (data: any) => {
    try {
      let response;
      if (name == 'admin') {
        response = await adminReset(data).unwrap();
      }
      if (name == 'super-admin') {
        response = await reset(data).unwrap();
      }
                    ...
                    <div className="relative">
                      <input
                        ...
                        {...register('otp', {
                          ...
                          maxLength: { value: 6, message: 'Invalid formate' },
                        })}
